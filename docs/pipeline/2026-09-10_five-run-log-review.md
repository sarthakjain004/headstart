...
> raises `HTTP Error 400` in the other 3, and `board_cost.csv` measures it at 562 s — at the
> 12 tech jobs it is credited, 1.28 tech/min, which fails the gate's own 2.0 threshold. The docstring's own survey found only 12
