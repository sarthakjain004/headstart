...
| `merge` | `embed_merge` → `update_meta` → `index sync` → `index prune` → `embed_prune` → `role_trends` → `company_directory` → embedding-store upload → `index refresh-indexes` → LanceDB upload |
