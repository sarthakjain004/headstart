...
(`data/descriptions/{ats}/*.jsonl.gz`, ~292 MB), pulled once at the start of the join job. A
...
That is not over-engineering. Rewriting the whole store every run would mint ~174 MB of fresh blobs
*per run*, and HF keeps every blob forever — the exact mistake that filled the 100 GB quota in ~45
...
zoho's. Every one of them is correctly *not* settled, which is why removing the settle branch does
