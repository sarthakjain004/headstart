...
our `employment_type_filter.FILTERS` substring matching makes unnecessary — we keep the provider's own
...
| lever | `commitment` → normalized `EmploymentType` enum | NOT-APPLICABLE — we keep provider wording, `employment_type_filter.FILTERS` matches by substring |
