...
- **Retries.** Theirs: 3 attempts, `Retry-After` honoured, exponential on 429 / linear on 5xx (`eightfold.py:311-335`). Ours: `headstart/http.py` retries 403/405/429/5xx with jittered backoff, a capped honoured `Retry-After`, per-reason counters, **plus** a WARP second-egress escalation (`egress_fallback_on = frozenset({403, 405, 429})`, `eightfold.py:120`). Ours is strictly richer.
