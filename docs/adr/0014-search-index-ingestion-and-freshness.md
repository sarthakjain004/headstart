...
The AI search layer (ADR-0005–0008) has never ingested the product's own output. `embed_sidecorpus.py`
reads a single hardcoded `data/jobs/sidecorpus.csv` — a one-off side-corpus — and `build_index.py`
...
1. **Wrong corpus.** The 18 ATS scrapers write canonical `Job` records to `data/jobs/{ats}.jsonl`,
   ...
   is measured over the Sidecorpus side-corpus, not the pipeline's. ADR-0007 already calls the
...
- **Not yet materialised.** No `{ats}.jsonl` is on disk (only one-off `sidecorpus.csv` / `zoho.csv`
...
- **Eval is Sidecorpus-locked.** Every qrel/label id is `sidecorpus:…`; moving the served index off
  Sidecorpus orphans the ADR-0011 labels unless Sidecorpus's role is decided.

## Decision

Four decisions, taken together.

**1. Served corpus = the pipeline's `{ats}.jsonl`; Sidecorpus becomes a frozen eval benchmark.**
...
index is built from `{ats}.jsonl`; the Sidecorpus CSV is retained, behind its existing adapter, purely
as the *labelled eval benchmark* — a fixed test set is meant to be stable, not the live corpus.
`embed_sidecorpus.py` is generalised to `embed_jobs.py --source`.
...
- **One unified index (Sidecorpus + pipeline together).** Simplest single table, but production would
  serve a stale one-off Sidecorpus scrape mixed with fresh Jobs, and the eval pool would be diluted by
  non-Sidecorpus neighbours — silently changing what nDCG = 0.90 means.
- **Re-scrape Sidecorpus into canonical `{ats}.jsonl`, drop the CSV.** Cleanest end-state, but voids
...
temporary adapter), surviving only for the Sidecorpus benchmark CSV. `build_index.py`'s overwrite is
