...
needs no band. Critique #2 measured about 3,400 served duplicate rows on v65 that the band left
out. The file has no writer: its rows were copied by hand from that sweep, so this amendment adds
the missing rows by hand from the v65 evidence. Nothing regenerates the file, so no re-run can
drop them.

**25 pairs added, 22 fronts** (Lumen stays out, the user's decision). They are Lockheed
(SuccessFactors), Ford, Ralliant, Fortive and Omnicell (Oracle), HSBC's `portal.careers.hsbc.com`,
Faurecia, Bayer, FCX, Insight, International SOS, Estée Lauder, LG CNS, Australian Unity and
Deloitte (SuccessFactors), and Plexus, AstraZeneca, Haleon, Kraft Heinz and Strada (Workday).
...
writer has no Lever or Jibe reader, so those two fronts are never aliased. The row rule covers
them.
