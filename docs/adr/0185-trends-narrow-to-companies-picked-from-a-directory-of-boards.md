...
- **A Board is not a company.** Workday splits one account into sites, Taleo Enterprise into
...
- **It lists every Board the delta ledger has counted** with tech `stock` at the live centroid
  ...
  a Board that closed its last opening still carries history. On 2026-09-24, 1,087 of the
  ledger's 34,203 Boards had no tech opening left, 59 of them under a company still hiring, and
  listing only current Boards would have dropped their history. It also means a company that
  has stopped hiring can still be picked.
...
**Two Boards are one company only when they are one ATS account or share a curated alias.** An
account is structural, read by `board_operator.tenant`, now public and shared with the Hot tab's
operator labels: a Workday `{company}`'s sites (684 accounts), a Taleo Enterprise host's career
sections (36 hosts), a Taleo Business Edition `org`'s `cws` sites (12 orgs, whose shared pod host
had previously stood for the account and moved no operator label when corrected), and casing
duplicates (`smartrecruiters:AbhiBus` / `abhibus`, 54 pairs). The only cross-ATS identity is
`board_naming.DISPLAY_ALIASES`, such as Lockheed Martin on Eightfold and SuccessFactors.
...
unrelated Ashby accounts. So an entry is a **Company** only as far as the data proves it. An
...
Hot-tab head surfaced 245 misnamed Boards, and the fixes change the Hot tab's names too:

- Taleo Enterprise's URL slugs tidied to "Https:" (81 Boards).
- SuccessFactors rows whose company is their host's first label ("Www", "Apply", "Join").
- Workday site names in the company column ("EXTERNAL_CAREERS" for Boeing, "CorporateCareers"
  for Mastercard).

A site segment is rejected as a name only when it is worded like a site, with a board word such
as "Careers" or "External". A bare site like `jiostar/JioStar` is often the best name available.
A few names get worse ("Samsung_Careers" becomes "Sec").
...
- **Company totals overcount wherever one company's Boards list the same requisitions, until
  ...
  - Taleo Enterprise career sections each serve the account's whole set: 22,929 rows for 2,475
    distinct (account, requisition) pairs. HDR alone has 8,235 rows for 549.
  ...
  whose requisitions another section already holds, keeps one row per (Workday account,
  requisition) in `sync` and `prune`, and later parks the Eightfold side of cross-ATS pairs.
  None of it re-keys a Board. Until it lands, PR 2 must not present an entry's sum as the
...
- On 2026-09-24 the directory holds 32,597 companies over 34,203 Boards, 830 of them owning more
  than one Board, in 2,073,491 bytes.
