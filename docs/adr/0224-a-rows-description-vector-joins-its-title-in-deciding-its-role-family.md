...
the 1,201 gold Jobs at weight 20, with the cutoff at 0.4. Then the sealed test was read, once:
...
- **The owner shipped anyway,** on four grounds:
  - both independent reads agree on +0.030;
...
1. **The head reads two inputs.**
   ...
   - **The manifest names the description input:** a `row_vector` block gives the embedder and
     width.
   - **`role_trends` refuses a mismatch.** A head whose `row_vector` does not match the served
     table's embedder and width errors, and writes no rows, instead of scoring vectors it never
     learned.
2. **The logits split, so serving stays cheap.** Because the head is linear, a row's logits are:
   ...
   `data/state/role_title_families.parquet`, which keeps its name. It now holds one logit per
   family instead of a decided family. The row part is one matrix product over the served vectors
   each run.
...
- **Rows that share a title still mostly share a family.** On snapshot v654, over titles with at
  ...
  The movers go mostly to non-tech and software-engineering: the "Systems Engineer at a utility"
  case the change is for.
