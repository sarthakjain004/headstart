...
got wired: `embed_sidecorpus.py` still embeds the one-off Sidecorpus CSV, `build_index.py` still
`create_table(mode="overwrite")` on a `sidecorpus` table, and `TABLE = "sidecorpus"` is baked into the
...
1. **Embed source = `data/jobs/tech/`** (reconciles ADR-0014's intent with ADR-0017). Generalise
   `embed_sidecorpus.py` → `embed_jobs.py --source`, reading via `corpus.iter_jobs`, keeping the
...
2. **A production `jobs` table; Sidecorpus becomes the frozen eval benchmark.** `search.py` gains
   `PROD_TABLE = "jobs"` + `EVAL_TABLE = "sidecorpus"`; `serve.py` serves `jobs`, the eval scripts and
   `search_sidecorpus.py` stay on `sidecorpus`. This makes ADR-0014's "Sidecorpus = benchmark" split
   concrete.
...
4. **`min_years`/`max_years` computed inline into `meta.jsonl`** (via `experience.extract(field,
   ...
   artifact and no join — Sidecorpus only needed that because extraction post-dated its embeddings.
...
- **A separate `data/enrich/tech` experience artifact + join** (mirroring Sidecorpus) — needless
...
The AI layer finally serves the product's real ~49.7k-Job tech corpus, and Sidecorpus is explicitly
