...
source's structured field (Sidecorpus provides `years_experience` for ~64% of Jobs) or, when that's
...
Output (`{id, min_years, max_years, source}`) is written to `data/enrich/sidecorpus_experience.jsonl`
...
**This coverage will fall as the corpus grows, by design.** Tier 1's 64% is Sidecorpus-specific — it
depends on Sidecorpus exposing a field. Field-less sources fall through to the regex tier (the
