...
**Spare egress**:
A second network origin a shard moves to once it has spent an ATS's **Origin budget** — Cloudflare WARP in proxy mode, dialled lazily by `headstart.warp` and routed by `http.fetch`'s `egress_group` (ADR-0063). It exists because a spent budget is the one failure retrying cannot fix: the same Boards serve 200 from another IP moments later, so every Board the shard has left is lost to an accident of which IP it drew. Opt-in per scraper (`BaseScraper.egress_fallback_on`, Eightfold's `{403, 405}`) and strictly a fallback — it never replaces per-host pacing, and a 429 deliberately does not trigger it.
_Avoid_: "rotation" or "IP rotation" — those describe cycling addresses to evade a limit; this moves once, only on a wall an ATS raised instead of a rate-limit signal, and stays put.
