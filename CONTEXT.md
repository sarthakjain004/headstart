...
A **Search filter** whose verdict is also stored per Job as a served column, so the where-clause tests a flag instead of re-deriving it. It answers the same Jobs whether the column exists or not: a table that predates the column keeps the raw fallback clause. Five exist: experience ceiling, employment type, India (`country`), salary known, and the posting-date shape guard. Each has one module that owns its columns, its verdict, its clause and, where it has one, its **Facet**'s options.
_Avoid_: index flag, cached filter. The column is a stored verdict, not an index, and it is never stale in a way the fallback would not be.
