...
**Role family**:
One of the curated groups the Trends tab partitions tech **Jobs** into (`config/role_families.json`): one axis, what the **Job** does — software-engineering, frontend-web, data-engineering, ai-ml-data-science and so on, plus `unclassified-tech` and the reserved `non-tech` diagnostic. Every served **Job** lands in exactly one. A trained classifier head decides it (`headstart.ingest.role_family_classifier`, trained by `scripts/embed/train_role_family_classifier.py`): since ADR-0220 from the title's JobBERT-v2 embedding, and since ADR-0224 from that plus the **Job**'s own served description `vector`, so a title that means different work in different industries ("Systems Engineer" at a utility) follows its description. A **Job** the head cannot place with confidence lands in `unclassified-tech` instead of a guessed family. Language, level and employer are never a family: Java and Python are watch roles, a tech lead takes the function of the work, and the employer's industry decides nothing. The list itself was measured: two blind labellers agreed on 88.8% of 250 uniform served **Jobs** against a finer list, and the one pair they could not separate (ML engineering and data science) became one family. Copies of a posting share a title and nearly always a family (1.5% of rows left their title's majority family on snapshot v654), and a retitled or re-described **Job** can change family. The trends ledgers' `version`, and the `centroid_version` key their snapshots carry, hold a series version (`role_trends.series_version`), not a fit's; the epoch ledger's `centroid_version` column reads `none` since ADR-0220.
