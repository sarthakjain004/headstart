...
**Scrapable Board** — 129,485:
A Unique Board a run may actually pick: minus `registry.DISABLED_ATS` (−25,488, all of it `join`), `config.EXCLUDED_BOARDS` (−50 vendor test Boards), the alias ledger (−412 Boards published under a second hostname or label, ADR-0111 and ADR-0182) and `config.PARKED_BOARDS` (−7). Computed by `load_active_companies(min_jobs=0)` — which applies these in the *other* order, excluding before it dedupes, and lands on the same figure. The right default answer to "how many Boards do we have".
_Avoid_: calling this "unique" — the 25,957 Boards between it and Unique Board are real and distinct, deliberately skipped rather than deduplicated. The alias subtraction is the one exception, and it is small: those 412 genuinely are not distinct Boards, they are one Board reached by more than one name.

**Hiring Board** — 84,224:
A Scrapable Board with at least one open posting (`load_active_companies(min_jobs=1)`, the function's default). The other 45,261 are live but empty.
