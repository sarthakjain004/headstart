...
A Unique Board a run may actually pick: minus `registry.DISABLED_ATS` (−25,488, all of it `join`), `config.EXCLUDED_BOARDS` (−44 vendor test Boards), the alias ledger (−400 Boards published under a second hostname or label, ADR-0111 and ADR-0182) and `config.PARKED_BOARDS` (−7). Computed by `load_active_companies(min_jobs=0)` — which applies these in the *other* order, excluding before it dedupes, and lands on the same figure. The right default answer to "how many Boards do we have".
_Avoid_: calling this "unique" — the 25,939 Boards between it and Unique Board are real and distinct, deliberately skipped rather than deduplicated. The alias subtraction is the one exception, and it is small: those 400 genuinely are not distinct Boards, they are one Board reached by more than one name.

**Hiring Board** — 83,474:
A Scrapable Board with at least one open posting (`load_active_companies(min_jobs=1)`, the function's default). The other 45,110 are live but empty.
