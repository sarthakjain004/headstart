...
**Dedup eviction** (ADR-0210):
An **Eviction** that is not a closure: `index prune` took a row out because another row serves the same posting — a casing variant of its Board, another site of its Workday **Tenant**, the Board behind an Eightfold career site, or a Board the alias ledger buries it onto. Recorded per run, Board and rule in `data/state/dedup_evictions.csv`, under the `ts` `role_trends` stamps, so Trends can add it back. Closures (sync) and other off-Board evictions are not recorded.
