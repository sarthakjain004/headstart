...
**Description store**:
The persisted text of every tech **Job**'s description, kept so it survives a run whose **Detail pass** failed (ADR-0050). Before it, a description was read once — at embed time, to build the **Doc** — and then discarded, so a Job scraped without one was embedded from its title alone and stayed that way permanently. Membership means one thing: *we hold this description*. A Job absent from it is fetched again. It briefly recorded a second fact — *this posting genuinely has none*, gated on a `Job.detail_fetched` flag — which ADR-0089 removed: nobody has found a Job in that state (sampled 2026-08-26, all 323 empty descriptions across 3,443 live Jobs on 63 boards traced to a failed fetch, against 7 such entries accumulated out of 328,930), and the flag was set by one scraper of the nine with a detail pass, so an empty description is in practice always a fetch that failed. Legacy `null` entries read as unheld and `compact` drops them. Written and read on CI in the join stage, never shipped to a scrape shard.
