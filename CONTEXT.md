...
`headstart.boards.board_identity` is the one module that turns a `CompanyRef` into one: `board_key()` is
...
**Salary**:
The compensation range a Job states, extracted to numbers so it can eventually be filtered on — a floor with an optional ceiling, open-ended when only a minimum is stated. Every figure is **period-normalized** to an annual amount (an hourly or monthly figure is annualized) but stays in its **native currency**, carried as its own field — a $ figure and a ₹ figure are never compared numerically *in the index*, so there is no converted column and no single "min salary" number spanning currencies. **The salary *filter* does compare across currencies** (ADR-0117): it restates the user's bounds in each currency at a committed dated rate, leaving every stored figure untouched — so "no cross-currency conversion" describes the data, not the query. The number is either **stated** in a structured field or read out of the description (`source` records which); unlike **Required experience**, there is deliberately **no seniority-style fallback estimate** — a fabricated salary risks misleading a real financial decision in a way an experience floor-estimate doesn't. Only a Job with no stated number anywhere is **unknown**, and unknown is not treated as exclusionary — it passes any salary-related filter rather than being hidden (ADR-0082).
_Avoid_: assuming `salary IS NOT NULL` (the `has_salary` filter) means "well-paid" — it is presence-only, not a number; treating the raw `salary` display string as a number before the parsed `min_salary_annual`/`max_salary_annual` columns exist for a given Job; assuming a bare `$` always names USD — it does when read from a description, but a bare `$` in a free-text salary field names no currency (ADR-0197).
...
# for board_cost specifically, read it through the loader, which is its canonical accessor:
python -c "from headstart.boards.cost_ledger import load; print(len({k.lower() for k in load('data/state/board_cost.csv')}))"
```
Count distinct keys, never lines — both files carry case-variants.

Two rules resolve most of it. **"live" describes a _row_, not a Board** — a sentence saying "live boards" is ambiguous by construction, because 6,632 live rows are duplicate spellings of a Board counted elsewhere. And **the subtractions depend on the order you apply them**: `EXCLUDED_BOARDS` removes 176 Boards from the raw live rows but only **173** from the deduped set, because three of them were themselves duplicate spellings. The chain below dedupes *first*; the README's funnel excludes first and so reads −176 / −6,629. Both reconcile; neither is quotable without saying which order it used.
...
A Board with a measured cost row in `data/state/board_cost.csv`, i.e. one we have read at least once. **Distinct Boards, not rows**: the file holds 85,839 lines, of which 1,945 are ADR-0023 case-variants of a Board already counted and 14 are two URL spellings of one Workday Board, so a `wc -l` reproduces the very conflation this section exists to kill. Load it through `cost_ledger.load()` and fold case. (It re-keyed pre-ADR-0096 rows until 2026-09-09; that migration is complete, so the loader now reads the key verbatim.) Some Scrapable Boards have never been scraped at all; that backlog is what the Tail drains.
...
**Dedup eviction ledger** (ADR-0210):
`data/state/dedup_evictions.csv`: one row per run, Board and rule counting the rows `index prune` took out because another row serves the same posting — a casing variant of its Board, another Board of its Workday, Taleo or ADP **Tenant**, the Board behind an Eightfold career site, or a Board the alias ledger buries it onto. Stamped with the `ts` `role_trends` stamps, so Trends can add them back. It never records an **Eviction** — a closed posting's removal, sync's alone — nor an off-Board removal for any other reason.
_Avoid_: reading "eviction" in its name as this glossary's **Eviction**; the rows it counts are dedups, and the name is the ledger's file name.
...
**Search filter**:
A structured constraint the user sets themselves on the **Search index** — remote, employment type, `min_years`, recency — compiled into a deterministic where-clause that runs *before* ranking. The full vocabulary is `headstart.search_filters.compiler.SearchFilters` (ADR-0149), one dataclass field per constraint — deliberately holding nothing about which ATSes, currencies or migration-only columns the currently-open Search index happens to carry, which is `IndexCapabilities` instead (a runtime fact of the index, not a Search filter, so it is never in `alerts.store.ALLOWED_SEARCH_FILTERS`'s vocabulary and can never be the **Blocking filter**). A **Subscription** carries a set of Search filters, and they are the only kind of filter left: the retired keyword bot's per-Subscriber `Filter` over the **Feed** went with it (ADR-0038).

**Materialized Search filter** (ADR-0173, ADR-0193):
A **Search filter** whose verdict is also stored per Job as a served column, so the where-clause tests a flag instead of re-deriving it. It answers the same Jobs whether the column exists or not: a table that predates the column keeps the raw fallback clause. Four Search filters are materialized: experience ceiling, employment type, India (`country`) and salary known. So is the shape guard that every posting-date filter carries. Each one's columns, verdict, clause and, where it has one, its **Facet**'s options live in one module. A stored verdict is as old as the row: a rule change reaches the fallback at once, but reaches the column only when rows are rewritten.
_Avoid_: index flag, cached filter. The column is a stored verdict, not an index.

**Keyword filter** (ADR-0104):
A **Search filter** that requires a typed keyword to appear, as a case-insensitive substring, in one of the served text columns its *scope* names — `title`, `description`, or both (`headstart.search_filters.compiler.KEYWORD_SCOPES`; a new scope is one map entry, its columns and its label — the rail's options, the `kw_in` whitelist, the disabled-until-migrated rule and the disclaimer all derive from the map). Several words are AND'd: each must appear somewhere in the scope. It narrows, the **Query** ranks — the two are independent controls. The description scope reaches only Jobs whose description is *stored* in the served table, which none indexed before the column existed are, so the UI carries a quantified disclaimer (rows with a description ÷ rows the other filters match — the keyword lifted, as a **Facet**'s "Any" row lifts its own dimension; both decided by the where-clause).
...
What a Trends line reports over a window, whole and reconciled: its start and latest openings, its hiring move, and its "Not hiring" split into named causes — each **Counting change**, **Found Board**s, duplicate postings removed, and growth counted twice before a removal — so that latest − start is exactly hiring plus those causes. A line's Marked changes are its causes; a breakdown's rows add up to its first row, with a closing row when a counting change reassigned more jobs than a category held. A category, level or role line is _mostly re-counted_ when its counting changes took out more than was left of its start, or under 5 openings were left: it then gives its hiring in openings and no percentage, index line or tile. A company's own line never is: the netting keeps it sound, since a counting change mostly moves jobs between a company's categories, and Hot ranks by it (ADR-0238).
...
**Tailoring** (ADR-0124, ADR-0133):
One job application's version of a **Résumé document**: which **Component**s it rewords, which it leaves out, and the **Job** it was written for where one is known — the tab offers that by listing the Account's **Saved job**s when a version is made, so "where one is known" means "made from a star". Of the Job it keeps the id and nothing else: the Tailoring's own name, which defaults to `Company · Title` and is the visitor's to change — at creation, or through Rename in the same group afterwards — is what still reads once the star is gone, so the posting is never copied a second time. It stores *differences*, never a copy — so a sentence fixed on the master still reaches every Tailoring that never disagreed with it, which is the entire reason it is not a duplicate document. A document with no Tailoring active is showing its **master**.
...
`data/state/role_centroids/manifest.json` (the retired centroid store's provenance, ADR-0220) are unrelated
...
The per-Board _measured_ scrape wall time in `data/state/board_cost.csv`, the cost estimate the scrape planner bin-packs on (ADR-0027). Keyed by `board_identity` in the casing its scraper builds (`cost_ledger.key_for`, ADR-0192), the same key as the **Board-priority ledger** since ADR-0096 — they disagreed until 2026-08-28, this one written under the scraper's raw `{ats}:{slug}` (a whole careers URL for Workday, pod included), so the two named the same Boards unjoinably and a tenant moving between pods orphaned its own cost history. Every scrape shard times each Board it scrapes and streams the row to its fragment; the join EWMA-blends them (`0.5·now + 0.5·history` — wall time is noisier than a Job count, so it leans on history harder than the priority ledger does). A Board with no measurement yet is estimated from its ATS's median, never from one global constant. A Board still mid-fetch when a shard's time budget kills it is costed for the seconds it burned, marked `unfinished` in the fragment — a **floor**, not a measurement, but the only evidence a kill leaves. A floor takes `max(stored, burned)` instead of the EWMA, because a bound may raise a Board's price and never lower it; without any of this a Board too big to finish writes no row at all, keeps its stale price, and is packed cheap into another shard every run (ADR-0064). The row's `jobs` count is the other half of that idea and was missing it until ADR-0145: an **errored** run is marked `errored` in the fragment and, like an unfinished one, may refresh the seconds but never overwrite a count a *complete* run learned — `harvest` records its fresh-id count as 0 whatever the outcome, so its 0 means "we never found out". Where no complete run ever has, the count is **empty** rather than 0, i.e. _unknown yield_. That distinction is load-bearing rather than cosmetic: the ADR-0064 value gate drops a Board whose last complete scrape found nothing, and a 0 that merely meant "this run raised" would exclude a healthy giant for a fortnight.
_Avoid_: confusing it with the **Board-priority ledger** — priority is a product question (which Boards deserve the slice), cost is an operational one (how to balance the shards). They decay differently and are read by different stages. They do now share a key, so joining them is finally meaningful; before ADR-0096 it silently produced nonsense for Workday and Personio. _Avoid_: reading a row as an upper bound — a Board killed unfinished is recorded as at-least-this-expensive, and its true cost is higher by an unknown amount. _Avoid_: reading `updated_at` as the age of the **jobs** count — it dates the last run that *looked at* the Board, which is what the gate's recheck wants, but an errored or unfinished look refreshes it while carrying the count forward, so a row can pair today's date with a days-old yield. _Avoid_: reading an empty `jobs` field as zero — empty is *unknown*, and only a real 0 is a finding.

**Value gate (cost per tech Job)**:
The rule that keeps one Board from owning a shard's hour: a Board whose *measured* cost exceeds 10 minutes (15 until 2026-09-24, when shards had come to finish in ~9) must carry at least 2 points of **Board-priority ledger** score per minute of shard time — its decayed tech-Job estimate, not a raw count, or `scrape_plan` leaves it out of the slice (ADR-0064). It exists because a shard's wall clock is set by its single slowest Board — **bin-packing** balances the sum, but a Board is indivisible — and when it was set, the ledger showed only 12 Boards costing over 15 minutes, splitting cleanly into ones earning 5.7–146 tech Jobs a minute and ones earning 0.01–1.35. A gated Board's costing expires after 14 days and it is scraped again, so the gate is re-decided on fresh evidence rather than frozen forever.
_Avoid_: applying it to an unmeasured Board — its cost is its ATS's median, so gating on it would drop a Board for its provider's reputation before it had a record of its own. _Avoid_: reading it as a tech filter — it prices a Board's hour and never inspects a posting; `headstart.jobs.tech_filter` remains the only authoritative gate.

**Description-gap ledger (unsettled counts)**:
Per **Board**, how many of its already-embedded **Jobs** the **Description store** has never settled — `data/state/board_description_gap.csv`, recomputed from scratch each run (ADR-0062). Those Jobs' derived columns cannot be repaired, because re-deriving without the text a value came from could only downgrade it, so the only fix is to scrape the Board again — and the priority ordering never reaches Boards that have not earned a score. `scrape_plan` therefore reserves `GAP_FRAC` of the **Tail** for them, cheapest ATS class first. Keyed on the **board_key** shape like the **Board-priority ledger**, but **lowercased** (via `key_for`): 1,693 of 13,708 gap Boards matched the live slice only case-insensitively, and folding ADR-0023's case-variant pairs keeps one Board from becoming two half-counts. Two classes are counted _unreachable_ instead: a **Job** on a `DISABLED_ATS`, and one whose **Board** this run scraped without re-emitting it — that posting has expired off the Board, and `reconcile()` only ever acts on ids the current scrape returned, so no future scrape can settle it (#185). A Board the run did not scrape, and an **Unauthoritative Board**, are no evidence and keep their counts.
_Avoid_: reading it as a backlog the slice can fully drain — 166 of its Boards are dead, excluded or parked and can never be picked, and `join`'s rows leave by eviction, not repair. _Avoid_: treating a Board's presence in the slice as evidence the quota picked it; a gap Board also reaches the slice through the head or the Tail on its own.

**Derivation cascade** (ADR-0061, ADR-0146):
The four field-extractors — `headstart.jobs.experience`, `headstart.jobs.salary`, `headstart.search_filters.india_gazetteer`, `headstart.jobs.remote` — composed into a **Job**'s nine derived meta columns: `remote`, `country`, `min_years`/`max_years`/`experience_source`, `min_salary_annual`/`max_salary_annual`/`salary_currency`/`salary_source`. One composition, `headstart.ingest.derived_meta`, called by both `doc_prep.to_meta` (cold-start, at embed time) and `update_meta.refresh_row` (repair, at sweep/rederive time) — before ADR-0146 each wrote this composition out independently, with no test crossing them, so a change to what an extractor's result mapped to could silently reach one path and not the other. `update_meta.refresh_row` keeps its own policy around this cascade — whether to recompute a row at all, and the no-held-description fallback that avoids nulling a value the cascade needs text to reproduce — which this term does not name.
_Avoid_: conflating it with the _tiered_ fallback inside one family (field, then description, then a seniority floor — ADR-0018) — that is one extractor's own internal cascade; this term names the four-family composition layered on top of every one of them.
...
**Detail pass**:
The second fetch a scraper makes per **Job**, after the listing endpoint — the one that fills fields the listing omits, usually `description`. It is what makes an ATS expensive: one request per Job rather than one per **Board**, which is how a single provider comes to spend a whole **Origin budget**. A Scraper on `BaseScraper.run_detail_pass` (ADR-0201) states each Job's request (`detail_request`) and how to read the answer (`read_detail`), and the base runs the rest — gate, skip, transport, loss labels, the gap line; only the four it does not fit (workday, adp, cornerstone, gem) compose the same steps by hand. A Job whose detail we already hold is skipped (ADR-0048), except that ADP, Apple, Cornerstone, Eightfold and Phenom fetch each held Job again every 7 days so an edit is caught, and Zwayam does not (ADR-0211); the scrape layer is told only *that* we hold it, never that it is embedded, so scrapers never depend on the embedding stage. "Already hold" means the **Description store** has the text — until ADR-0050 it meant merely that the Job had been embedded, which is why a Job embedded without a description was skipped forever and could never be repaired. A second, separate skip gates on the **Tech filter** instead of the store: a posting the tech gate will discard is never indexed, so its detail is never worth fetching at all. Nineteen scrapers do it, all through one seam, `BaseScraper.tech_detail_wanted` (ADR-0166). Thirteen are **exact**: `parse` reads `title` and `department` off the same listing item the gate reads, and the detail overrides neither, so the gate's verdict *is* the verdict `filter_tech` will reach — workday, smartrecruiters, apple, trakstar, zwayam, pyjamahr (ADR-0175), peoplestrong (ADR-0234: `jobTitle` and `organizationUnit` both come off the listing, and the detail changed the title on 0 of 35,732 postings), pinpoint (ADR-0184, whose detail supplies only the date and country), clearcompany (ADR-0182: both fields come from HRM Direct's `xml.php`, and the detail page supplies only the body and a `Salary:` row), adp (ADP Workforce Now, ADR-0180: no department on either surface, and the detail adds only the description), adp_recruiting (ADP Recruiting Management, a separate product, ADR-0202: both fields come off the listing, whose description is whole, and the detail supplies only pay), cornerstone (ADR-0183: no surface states a department at all, and its job ad's `title` equals the listing's, 58 of 58, so both sides ask with the title alone) and eightfold, whose PCSX listing carries both (ADR-0048's 2026-09-16 amendment, which carries the measurements). Six are **measured approximations**, each a tolerance checked against real postings rather than a guarantee: successfactors' listing carries neither field, so it reads the gate off the job URL's own slug; jazzhr's carries both but its detail page may override `department`; phenom's carries both, and its detail *overrides* `title` while merely *backing up* `department`; gem's carries both and its detail backs up either; bamboohr's reads the department off the widget's own department headers, and its detail's `departmentLabel` — which `parse` prefers — matched that header on 28 of 28 postings of the one tenant measured live (2026-09-24; one of them states no department on either side); and rippling's detail is a `department` fallback, inert only because that field is empty on all 1,515 of its postings. taleo_enterprise and taleo_be were **measured dirty** (#510) and gate nothing, for two different reasons: taleo_enterprise's listing is a tenant-configured table that often carries no department column at all, so the gate loses 68 of 133 tech postings across the 10 Boards chosen as its worst — and 11 of 120 (9.2%) across 7 chosen without that bias, 3 of which lose nothing, making it a per-tenant cliff rather than a flat rate; taleo_be's carries one but reads it *positionally*, and the one sampled tenant that emits its head fields in the other order cost 5 of that Board's 16 — a per-tenant cliff, not a uniform loss. ripplehire is **deferred** rather than excluded — its title-only gate measures at 1.8% recall loss, too small to refuse outright and too costly to accept at 0.3% of board-seconds (ADR-0166 has every sample). oracle, zoho, icims and jobvite deliberately **do not** gate: their `department` arrives only on the detail, and `tech_filter` rule 4 promotes a vague title on a technical department, so a gate blind to it drops real tech postings — measured over the 2026-09-17 corpus at 46.0% of oracle's and 47.4% of zoho's. meta is excluded more simply than any of them: its sitemap states a bare numeric id and nothing else (994 of 994 probed), so it has no pre-detail signal at all — not a listing field, not a URL slug. Only some ATSes have a detail pass; where there is none, every field arrived with the listing and so cannot go missing.
_Avoid_: "enrichment" — the detail pass fetches primary fields, it does not derive them.

**Role family**:
One of the curated groups the Trends tab partitions tech **Jobs** into (`config/role_families.json`): one axis, what the **Job** does — software-engineering, frontend-web, data-engineering, ai-ml-data-science and so on, plus `unclassified-tech` and the reserved `non-tech` diagnostic. Every served **Job** lands in exactly one. A trained classifier head decides it (`headstart.ingest.role_family_classifier`, trained by `scripts/embed/train_role_family_classifier.py`): since ADR-0220 from the title's JobBERT-v2 embedding, and since ADR-0224 from that plus the **Job**'s own served description `vector`, so a title that names different work in different postings (a "Systems Engineer" whose description is power-grid work) follows its description. A **Job** the head cannot place with confidence lands in `unclassified-tech` instead of a guessed family. Language, level and employer are never a family: Java and Python are watch roles, a tech lead takes the function of the work, and the employer's industry alone decides nothing. The list itself was measured: two blind labellers agreed on 88.8% of 250 uniform served **Jobs** against a finer list, and the one pair they could not separate (ML engineering and data science) became one family. Copies of a posting share a title and nearly always a family (on snapshot v654, 1.5% of the rows of titles with at least three rows left their title's majority family), and a retitled or re-described **Job** can change family. Which head decided a tick's families is its **Methodology**'s `family_classifier_version` (ADR-0230); the series versions the trends ledgers once carried under `centroid_version` retired with ADR-0230 step 6.
_Avoid_: reading a family as a cluster — the centroids that decided families (alone until ADR-0215, as the fallback until ADR-0220) are retired — or as a language or level.

**Role watchlist**:
A curated set of specific roles tracked as their own trend lines, matched on **Job** title patterns rather than by embedding (ADR-0051, amended by ADR-0052 and ADR-0220). It names the facets a one-axis **Role family** list leaves out on purpose: a language (Java, Python), a named sub-role (Forward Deployed, Backend), or a specialty inside a family. It began as a workaround for centroids that could not express such roles, either too small to earn a cluster or, like Backend, large but split by seniority and phrasing rather than by domain; the centroids are retired, and the roles stayed. It counts only **Jobs** whose **Role family** is tech (ADR-0215), and each role sits under the family its title means (ADR-0220). A pattern buys two things a learned model cannot: you can say exactly why a Job counted, and a new classifier head leaves it untouched. A watched role is counted *in addition to* the family it belongs to, never instead of it.
...
**Description store**:
The persisted text of every tech **Job**'s description, kept so it survives a run whose **Detail pass** failed (ADR-0050). Before it, a description was read once — at embed time, to build the **Doc** — and then discarded, so a Job scraped without one was embedded from its title alone and stayed that way permanently. Membership means one thing: *we hold this description*. A Job absent from it is fetched again. It briefly recorded a second fact — *this posting genuinely has none*, gated on a `Job.detail_fetched` flag — which ADR-0089 removed because the flag is wrong on live data. Eightfold, the one scraper of the nine that ever set it, answers HTTP 200 with no description for postings whose pages carry full text (5 of 5 on `telekom-growthhub`, one of them tech), so the flag settled them "has none" permanently. Checking every `null` the state wrote over its lifetime — 8 entries, all eightfold, out of 417,773 store-wide as of 2026-08-26 — found 5 wrong, 1 right, 2 unverifiable. A completed fetch is not an authoritative answer. Legacy `null` entries read as unheld and `compact` drops them. Written and read on CI in the join stage, never shipped to a scrape shard.
_Avoid_: "cache" — a cache may be cleared, and clearing this one un-skips every **Detail pass** at once. Also distinguish it from the *embedding store*, which holds vectors; this holds text. _Avoid_: quoting the 2026-08-26 95-board / 13,061-Job sample as a rate — that draw was random and unseeded and is not reproducible, which is why ADR-0089 §"How rare the middle row actually is" describes the sample but prints none of its per-ATS counts; the reproducible facts are the 8-entry census above and the store's own size.
