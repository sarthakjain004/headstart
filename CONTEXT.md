...
It is **not** a second budget, and ADR-0067 measured why: Cloudflare pins a WARP client to the datacentre nearest its **GitHub VM**, that colo never changes for the life of the shard, and each colo carries only one to three egress IPs. So rotation redraws from a handful of addresses — the same one about half the time, and on a single-IP colo always — and across 30 shards the 30 *distinct* direct IPs collapsed onto 11 WARP IPs, one shared by eight shards. A walled shard therefore moves off an address nobody else is using and onto one its siblings may already be spending. It is still worth taking, because it is dialled only after the direct IP is already refused; it just buys a different address, not room.
_Avoid_: treating it as evasion. It moves only after the ladder is spent, only for an ATS opted in by name, and only where the metering was measured to be per source IP. Equally avoid reading it as a fresh allocation — see above; that was ADR-0063's premise and ADR-0067 retired it.
