...
**Description store**:
The persisted text of every tech **Job**'s description, kept so it survives a run whose **Detail pass** failed (ADR-0050). Before it, a description was read once — at embed time, to build the **Doc** — and then discarded, so a Job scraped without one was embedded from its title alone and stayed that way permanently. Membership means one thing: *we hold this description*. A Job absent from it is fetched again. It briefly recorded a second fact — *this posting genuinely has none*, gated on a `Job.detail_fetched` flag — which ADR-0089 removed: the state is real but tiny, and the flag feeding it is untrustworthy (sampled 2026-08-26 over 13,061 live Jobs on 95 boards, 399 empty descriptions of which only 9 — 1 of them tech — were a posting that genuinely has none, against 7 such entries accumulated out of 328,930). On most detail-pass ATSes an empty answer is not reliably distinguishable from a failed fetch, and the flag was set by one scraper of the nine with a detail pass, so settling on it would permanently record a falsehood for more Jobs than the state ever spared. Legacy `null` entries read as unheld and `compact` drops them. Written and read on CI in the join stage, never shipped to a scrape shard.
