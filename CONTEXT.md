...
**Role family**:
One of the curated groups the Trends tab partitions tech **Jobs** into (`config/role_families.json`, ADR-0040): software-engineering, data-engineering, qa-test and so on, plus the reserved `non-tech` diagnostic. Every served **Job** lands in exactly one. Since ADR-0215 the **Job**'s title decides it first: `headstart.ingest.role_family_rules` matches cues shaped like the **Tech filter**'s, and only a title no rule decides (about a quarter of served rows) falls back to its nearest frozen centroid, mapped through the curated file. The title came first because the centroids were fitted on title plus full description, so an employer's boilerplate decided the family as much as the role did: on 250 served **Jobs** hand-labelled after the rules were frozen, centroids alone got 60.0% right and the rules-first order 70.8%. A retitled **Job** can change family, and so can a re-embedded one whose title no rule decides. Because the rules decide first, the trends ledgers' `version`, and the `centroid_version` key their snapshots carry, hold a series version (`role_trends.series_version`: 2001 for centroid version 2 under the first rules generation), not the fit's own version; the `centroid_version` column of `trends_epochs` is still the fit's.
_Avoid_: reading a family as a cluster. Clusters are raw material the curated map groups, and since ADR-0215 most rows never consult one.

**Role watchlist**:
A curated set of specific roles tracked as their own trend lines, matched on **Job** title patterns rather than by embedding (ADR-0051, amended by ADR-0052). It exists for any role the centroid fit cannot express — whether too small to earn a cluster (a Forward Deployed Engineer is under 0.2% of the rows the fit clustered, so it smears across general software clusters) or large but unclusterable, like Backend, which k-means cannot separate because it split the software-engineering catch-all by seniority and phrasing rather than by domain. It counts only **Jobs** whose **Role family** is tech (ADR-0215), and each role sits under the family most of its **Jobs** land in. A pattern buys two things a centroid cannot: you can say exactly why a Job counted, and a centroid refit leaves it untouched. A watched role is counted *in addition to* the family it belongs to, never instead of it.
