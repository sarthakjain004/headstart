...
- **"Tenant" (retired)** — previously the `(ATS, slug)` pair. Dropped as a term: a **Company** *is* the thing on an ATS, located by its **Slug**, so we just say "a Company's slug on an ATS." The data still carries a `tenant` column (and the `data/ats-tenants-merged/` dir, `slug_from(tenant, …)` param keep the name) — a code/data rename is a separate change, not yet done. **Workday tenant** and **requisition** (ADR-0187) are Workday's own words and are used only in that qualified form: the tenant is the `{company}` segment of `workday:{company}/{site}`, and runs several sites, each its own **Board**; a requisition is one opening, identified by the **Job**'s native id, which the tenant can post to several of its sites. The **Search index** serves a requisition once per Workday tenant, not once per Board.
