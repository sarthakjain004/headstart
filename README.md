...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>132,993 live rows of 216,014<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 132,993 live *rows* reduce to 96,015 **Scrapable Boards** a run can even
consider (measured 2026-09-16; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 132,993 | a row, not a board — 6,632 of them are duplicate spellings |
...
−6,632, because two excluded boards were themselves duplicates. Both land on 96,015.
...
Of those, **61,441 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
the 34,574 live-but-empty boards are skipped as having nothing to read. `pick_boards` takes a
slice of
...
36 scrapers, selected from a registry by the `ats` key: `amazon`, `apple`, `ashby`, `bytedance`,
...
`uber`, `workable`, `workday`, `zoho`, `zwayam`, `jazzhr`, `jobvite`, `phenom`. `join` is in `registry.DISABLED_ATS` — German-SMB boards running ~1 tech job in
...
liveness pipeline has probed **216,014 ledger rows**: 132,993 live, 66,943 dead, 16,078 unknown —
rows, not boards; they collapse to 126,361 Unique Boards (CONTEXT.md §Counting Boards). Of the
36 scrapers, 19 have rows in the index — `sensehq` is a single-company unlock with nothing
