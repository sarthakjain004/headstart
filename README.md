...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>135,473 live rows of 229,565<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 135,473 live *rows* reduce to 103,227 **Scrapable Boards** a run can even
consider (measured 2026-09-16; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 135,473 | a row, not a board — 6,632 of them are duplicate spellings |
...
−6,632, because two excluded boards were themselves duplicates. Both land on 103,227.
...
Of those, **67,023 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
the 36,204 live-but-empty boards are skipped as having nothing to read. `pick_boards` takes a
slice of
...
liveness pipeline has probed **229,565 ledger rows**: 135,473 live, 78,012 dead, 16,080 unknown —
rows, not boards; they collapse to 128,841 Unique Boards (CONTEXT.md §Counting Boards). Of the
