...
them through **25 per-ATS scrapers**, normalizes everything into one `Job` shape, and serves it
...
25 scrapers, selected from a registry by the `ats` key: `ashby`, `bytedance`, `darwinbox`,
...
`tesla`, `trakstar`, `workable`, `workday`, `zoho`, `zwayam`. `join` is in `registry.DISABLED_ATS` — German-SMB
...
25 scrapers, 19 have rows in the index — `sensehq` is a single-company unlock with nothing
indexed yet, `zwayam` (2026-08-27), `icims` (2026-09-08), `oracle` (2026-09-08, which had a
scraper but no ledger until then), `tesla` (2026-09-11) and `bytedance` (2026-09-11) were added
