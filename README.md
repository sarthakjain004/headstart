...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>119,763 live rows of 184,559<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 119,763 live *rows* reduce to 87,661 **Scrapable Boards** a run can even
consider (measured 2026-09-06; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 119,763 | a row, not a board — 6,617 of them are duplicate spellings |
...
−6,617, because two excluded boards were themselves duplicates. Both land on 87,661.
...
the 32,268 live-but-empty boards are skipped as having nothing to read. `pick_boards` takes a
slice of
...
liveness pipeline has probed **184,559 ledger rows**: 119,763 live, 57,316 dead, 7,480 unknown —
rows, not boards; they collapse to 113,146 Unique Boards (CONTEXT.md §Counting Boards). Of the
