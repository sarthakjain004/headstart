...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>131,888 live rows of 206,796<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 131,888 live *rows* reduce to 94,965 **Scrapable Boards** a run can even
consider (measured 2026-09-11; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 131,888 | a row, not a board — 6,632 of them are duplicate spellings |
...
−6,632, because two excluded boards were themselves duplicates. Both land on 94,965.
...
Of those, **60,577 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
...
liveness pipeline has probed **206,796 ledger rows**: 131,888 live, 66,360 dead, 8,548 unknown —
rows, not boards; they collapse to 125,256 Unique Boards (CONTEXT.md §Counting Boards). Of the
