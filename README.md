...
them through **28 per-ATS scrapers**, normalizes everything into one `Job` shape, and serves it
...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>131,883 live rows of 206,791<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
    ...
    subgraph P["② Ingest &nbsp;·&nbsp; GitHub Actions, back-to-back &nbsp;·&nbsp; ADR-0025 / ADR-0026"]
        ...
        P2["<b>scrape</b><br/>≤15 VMs · 60m budget<br/>25 enabled scrapers → fragments"]
...
that matters. The 131,883 live *rows* reduce to 94,960 **Scrapable Boards** a run can even
consider (measured 2026-09-11; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 131,883 | a row, not a board — 6,632 of them are duplicate spellings |
...
−6,632, because two excluded boards were themselves duplicates. Both land on 94,960.
...
Of those, **60,572 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
...
28 scrapers, selected from a registry by the `ats` key: `amazon`, `apple`, `ashby`, `bytedance`,
...
scraper but no ledger until then), `amazon`, `bytedance` and `apple` (all 2026-09-11, Single
source scrapers, ADR-0139) were added since the last pipeline run and have nothing indexed yet,
...
- `src/headstart/` — shared library, used by both the pipeline and the curated feed: `models.py`
  (Job + normalization), `scrapers/` (25 per-ATS + `base`/`registry`), `http.py` (the pooled
