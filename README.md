...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>126,110 live rows of 189,409<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 126,110 live *rows* reduce to 89,201 **Scrapable Boards** a run can even
consider (measured 2026-09-06; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 126,110 | a row, not a board — 6,619 of them are duplicate spellings |
...
−6,617, because two excluded boards were themselves duplicates. Both land on 89,201.
...
Of those, **56,797 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
the 32,404 live-but-empty boards are skipped as having nothing to read. `pick_boards` takes a
slice of
