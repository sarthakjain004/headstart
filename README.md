...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>132,424 live rows of 208,559<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 132,424 live *rows* reduce to 95,446 **Scrapable Boards** a run can even
consider (measured 2026-09-11; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 132,424 | a row, not a board — 6,632 of them are duplicate spellings |
...
−6,632, because two excluded boards were themselves duplicates. Both land on 95,446.
...
Of those, **60,931 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
the 34,515 live-but-empty boards are skipped as having nothing to read. `pick_boards` takes a
slice of
...
31 scrapers, selected from a registry by the `ats` key: `amazon`, `apple`, `ashby`, `bytedance`,
...
`successfactors`, `teamtailor`, `tesla`, `tiktok`, `trakstar`, `uber`, `workable`, `workday`,
`zoho`, `zwayam`. `join` is in `registry.DISABLED_ATS` — German-SMB boards running ~1 tech job in
...
liveness pipeline has probed **208,559 ledger rows**: 132,424 live, 66,802 dead, 9,333 unknown —
rows, not boards; they collapse to 125,792 Unique Boards (CONTEXT.md §Counting Boards). Of the
31 scrapers, 19 have rows in the index — `sensehq` is a single-company unlock with nothing
