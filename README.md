...
Discovery. 42 scrapers. Embeddings. Vector search. Email and Telegram alerts.
...
**42 scrapers**, selected from a registry by the `ats` key: `amazon`, `apple`, `ashby`,
...
Eight of the 42 — `amazon`, `apple`, `bytedance`, `google`, `meta`, `tesla`, `tiktok`, `uber`
...
The liveness pipeline has probed **275,160 ledger rows**: 162,074 live, 96,930 dead, 16,156 unknown
— rows, not boards; they collapse to 155,442 Unique Boards once duplicate spellings of the same
...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>162,074 live rows of 275,160<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
    ...
    subgraph P["② Ingest &nbsp;·&nbsp; GitHub Actions, back-to-back &nbsp;·&nbsp; ADR-0025 / ADR-0026"]
        ...
        P2["<b>scrape</b><br/>≤15 VMs · 60m budget<br/>41 enabled scrapers → fragments"]
...
| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 162,074 | a row, not a board — 6,632 of them are duplicate spellings |
...
because two excluded boards were themselves duplicates. Both land on 129,485.

Of those, **84,224 are currently hiring** — the 45,261 live-but-empty boards are skipped as having
...
- `src/headstart/` — shared library, used by both the pipeline and the curated feed: `models.py`
  (Job + normalization), `scrapers/` (42 per-ATS + `base`/`registry`), `http.py` (the pooled
