...
Serving:
  ...
  semantic search → embed → LanceDB → filter-then-rank + Telegram alert bot
    (built + evaluated on the Sidecorpus benchmark corpus; embedding the tech subset is next, ADR-0019)
```
...
python scripts/embed/embed_sidecorpus.py   # embed the English corpus → data/embeddings/
