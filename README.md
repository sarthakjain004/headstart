...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>122,917 live rows of 181,979<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 122,917 live *rows* reduce to 86,063 **Scrapable Boards** a run can even
consider (measured 2026-09-06; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 122,917 | a row, not a board — 6,619 of them are duplicate spellings |
| − `registry.DISABLED_ATS` | −30,166 | **all of it `join`** — German-SMB boards at ~1 tech job in ~10k |
...
That order matters: excluding before deduping reads −45 and −6,617, deduping first reads −43 and
−6,619, because two excluded boards were themselves duplicates. Both land on 86,063.
...
Of those, **54,224 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
the 31,839 live-but-empty boards are skipped as having nothing to read. `pick_boards` takes a
slice of
...
liveness pipeline has probed **181,979 ledger rows**: 122,917 live, 51,901 dead, 7,161 unknown —
rows, not boards; they collapse to 116,298 Unique Boards (CONTEXT.md §Counting Boards). Of the
