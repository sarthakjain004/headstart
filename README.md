...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>133,633 live rows of 217,902<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
that matters. The 133,633 live *rows* reduce to 101,387 **Scrapable Boards** a run can even
consider (measured 2026-09-15; the terms are defined in `CONTEXT.md` §Counting Boards):

| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 133,633 | a row, not a board — 6,632 of them are duplicate spellings |
...
−6,632, because two excluded boards were themselves duplicates. Both land on 101,387.
...
Of those, **65,925 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
the 35,462 live-but-empty boards are skipped as having nothing to read. `pick_boards` takes a
slice of
...
liveness pipeline has probed **217,902 ledger rows**: 133,633 live, 68,190 dead, 16,079 unknown —
rows, not boards; they collapse to 127,001 Unique Boards (CONTEXT.md §Counting Boards). Of the
