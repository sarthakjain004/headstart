...
that matters. The 117,708 live *rows* reduce to 85,607 **Scrapable Boards** a run can even
...
| − `config.EXCLUDED_BOARDS` | −44 | vendor test/sandbox boards, confirmed by reading their postings |
...
−6,617, because two excluded boards were themselves duplicates. Both land on 85,607.
...
Of those, **53,811 are currently hiring** — `load_active_companies` defaults to `min_jobs=1`, so
