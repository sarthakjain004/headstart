...
boards, scrapes them through **36 per-ATS scrapers**, and normalizes every posting into one `Job`
...
**36 scrapers**, selected from a registry by the `ats` key: `amazon`, `apple`, `ashby`,
`bytedance`, `darwinbox`, `eightfold`, `freshteam`, `google`, `greenhouse`, `icims`, `jazzhr`,
`jobvite`, `join`, `keka`, `lever`, `meta`, `oracle`, `personio`, `phenom`, `recruitee`,
`ripplehire`, `rippling`, `sensehq`, `smartrecruiters`, `successfactors`, `taleo_be`,
...
Eight of the 36 — `amazon`, `apple`, `bytedance`, `google`, `meta`, `tesla`, `tiktok`, `uber`
...
(`ashby`, `eightfold`, `jobvite`, `keka`, `lever`, `phenom`, `ripplehire`, `taleo_enterprise` —
ADR-0114); every other ATS serves the **ATS slug** in that field instead, so a row's `company` may
be either.

The liveness pipeline has probed **229,573 ledger rows**: 135,517 live, 77,976 dead, 16,080 unknown
— rows, not boards; they collapse to 128,885 Unique Boards once duplicate spellings of the same
...
flowchart TB
    subgraph D["① Discovery &nbsp;·&nbsp; occasional, by hand"]
        ...
        D4[("<b>liveness ledger</b><br/>135,517 live rows of 229,573<br/>git-tracked, authoritative")]
        D1 --> D2 --> D3 --> D4
    end
...
| | boards | |
| --- | ---: | --- |
| live rows in the ledger | 135,517 | a row, not a board — 6,632 of them are duplicate spellings |
...
because two excluded boards were themselves duplicates. Both land on 103,270.

Of those, **67,060 are currently hiring** — the 36,210 live-but-empty boards are skipped as having
