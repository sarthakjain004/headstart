...
- **The back-to-back ingest run lives in `src/headstart/ingest/` — not in `scripts/`** (ADR-0028).
  ...
  `derived_meta`, `doc_prep`, `index_plan`, `observability`, `role_assignments`,
  `role_family_rules`, `shard_plan`, `shard_speedup`, `trends_epochs`). Logic the curated-feed
  path (`python -m headstart` → `headstart.harvest`) also reaches stays in `headstart` proper
