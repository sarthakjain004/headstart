...
Tenant lists per ATS â€” every company that **jobhive** scrapes lives in
...
- `name` â€” display name. Free-form. Used as `Job.company` until the
  scraper resolves a richer value from the live page.
- `slug` â€” the scraper/API identifier the matching scraper accepts
  (lowercase, deterministic). Pipelines should prefer this column when
  reading the CSV.
- `url` â€” the canonical public careers URL. User-facing â€” safe to
...
| mercor | seed file only â€” Mercor exposes no per-company endpoint |
...
When in doubt, look at the existing rows in the file you're editing â€”
...
- **Sort matters.** Files are roughly ASCII-sorted by `name` â€” drop
...
indexes â€” but those passes don't catch everything, and a manual PR is
...
- A few ATSes (`tesla`, `meta`) require a real browser and don't ship
  a tenant list â€” they'll come back here once the optional browser
...
- Public-sector aggregators (`bundesagentur`, `arbetsformedlingen`,
  ...
  `ycombinator`, `sidecorpus`, â€¦) are single-tenant â€” no CSV here.
