...
Only `data/validate/` (the liveness ledger and `board_moves.txt`) and
`data/ats-tenants-merged/active/` (and the eval labels) are committed — everything else regenerates
from the scripts. The served product reads the liveness
