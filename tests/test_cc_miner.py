...
def test_taleo_capture_keeps_all_board_coordinates(miner):
    ...
    second = "https://phe.tbe.taleo.net/phe02/ats/careers/v2/searchResults?org=ACME&cws=1"
    one, two = miner.tenant_from("taleo_be", pattern.search(first)), miner.tenant_from("taleo_be", pattern.search(second))
    assert one[0] != two[0]
    assert one[1] == "https://phe.tbe.taleo.net/phe01/ats/careers/v2/searchResults?org=ACME&cws=1"
