...
def test_india_expands_via_geo():
    ...
    values are unaffected by `has_country` (ADR-0120) — only the top-level "india" value ever
...
def test_india_country_level_uses_the_materialized_column_when_available():
    """ADR-0120: once the table carries `country`, the whole-country case uses a plain equality
...
def test_india_country_level_falls_back_to_geo_without_the_column():
    """Dark-until-migrated (ADR-0120): forgetting `has_country`, or a table that hasn't synced
...
def test_has_country_is_learned_from_the_schema():
    """ADR-0120, the same dark-until-migrated rule `has_description` follows above."""
    searcher, table = _searcher()
