...
def test_allowed_search_filters_stay_a_subset_of_the_real_vocabulary():
    ...
    `headstart.search_filters.compiler.SearchFilters` (the alerts run calls the Space over HTTP, against a
    deployed commit that may differ — see the comment beside `ALLOWED_SEARCH_FILTERS`). Nothing then stops the two from drifting apart on their own, so
    this is the automated check that catches it: a rename or typo in `search_filters/compiler.py`'s filter
    vocabulary now fails here, in CI, rather than silently dropping a filter from every Digest.
    """
    from headstart.search_filters.compiler import SearchFilters
