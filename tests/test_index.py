...
def _sync(
    ...
    return idx.sync(
        argparse.Namespace(
            ...
            # Same reason, and absent on purpose: no Board errored, so the scope is the
            # infer-from-lines one these tests were written against (ADR-0053).
            scrape_errors=str(tmp_path / "scrape_errors.json"),
        )
    )
