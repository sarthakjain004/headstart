...
def update(
    ...
    for board, held in sorted(unresolved.items()):
        age = (
            round(
                (at - datetime.fromisoformat(held.last_authoritative)).total_seconds()
                / 86400,
                2,
            )
        ...
        totals = by_ats.setdefault(
            ats,
            {
                ...
                "oldest_authoritative_run_days": None,
            },
        )
        ...
        if age is None:
            ...
            totals["oldest_authoritative_run_days"] = (
                age if prior is None else max(age, prior)
            )
        boards.append(
            {
                "board": live[board],
                **asdict(held),
                "authoritative_age_days": age,
    ...
    with gzip.open(temporary, "wt", encoding="utf-8", newline="") as target:
        ...
        writer.writeheader()
        writer.writerows(
            {"board": board, **asdict(held)} for board, held in sorted(history.items())
        )
    temporary.replace(path)
    (state_dir / "board_freshness_report.json").write_text(
