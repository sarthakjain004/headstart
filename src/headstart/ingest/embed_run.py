...
#
# 1. Attention memory scales with batch × seq² — a fixed batch of 8 (calibrated on Sidecorpus's
...
# doc transiently demands ~50 GB on this stack. 4,096 is inside the envelope the Sidecorpus run
...
def main() -> None:
    ...
    ap.add_argument(
        "--source",
        default=str(_SOURCE),
        help="corpus source: a {ats}.jsonl directory or a Sidecorpus CSV (default: data/jobs/tech)",
    )
    ap.add_argument(
        "--limit",
        type=int,
