...
from a Board this run scraped in full — both unreachable)`, then the drain line, then the top-10
boards by backlog with each one's own delta. This is the ADR-0050 description-store backlog:
`unreachable` ids can never settle (wrong ATS, or the Board's own authoritative scrape already
...
`U` is a **level**, so read the drain line and not it: `S settled, A newly unsettled, net ±X`
(ADR-0162). A frozen backlog and a fast-churning one print the same `U`, which is how five
consecutive runs of net growth went unnoticed. Both the drain line and the per-board deltas are
**absent on a run with no prior ledger**, and the `blocked` line is absent when the count is zero —
optional by design, so their absence is not a parse failure.
...
GAP_DRAIN = re.compile(
    ...
    r"this run read: ([\d,]+) settled, ([\d,]+) newly unsettled, net ([+-][\d,]+)"
)
...
def report(run: Run) -> None:
    ...
    if gh:
        ...
        if gd:
            was, was_boards, settled, arrived, net = gd.groups()
            print(
                f"  drain: {settled} settled, {arrived} newly unsettled, net {net} "
                f"(was {was} across {was_boards} boards)",
                flush=True,
            )
        else:
            print(
