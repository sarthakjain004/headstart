...
def main() -> None:
    ...
    ap.add_argument("--type", dest="employment_type", help="employment_type, e.g. full-time")
    ap.add_argument("--max-years", type=int, dest="max_years",
    ...
    if device == "mps":
        model = model.half()
    query_vec = model.encode([_QUERY_PREFIX + args.query], normalize_embeddings=True)[0].astype("float32")
    ...
    if filters:
        search = search.where(" AND ".join(filters), prefilter=True)  # filter first, then rank survivors
    ...
    for rank, r in enumerate(rows, 1):
        ...
        yrs = r.get("min_years")
        print(f"{rank:2}. {sim:.3f}  {r['title']} — {r['company']}  "
