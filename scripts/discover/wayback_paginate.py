...
def main():
    ...
    try:
        while True:
            ...
            if urls is None:
                print(
                    "fetch failed repeatedly — cursor saved, re-run to resume",
                    flush=True,
                )
                break
            pages += 1
            ...
            f.flush()
            print(
                f"page {pages}: scanned {len(urls)} urls, +{new} new "
            ...
            if nxt and (max_pages == 0 or pages < max_pages):
                ...
                if not nxt:
                    ...
                    print(
                        f"stopped at max_pages={max_pages} (cursor saved)", flush=True
                    )
                break
    finally:
        f.close()
