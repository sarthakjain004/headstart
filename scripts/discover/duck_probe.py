...
raw = subprocess.run(
    ["curl", "-s", "-m", "60", f"https://data.commoncrawl.org/crawl-data/{crawl}/cc-index-table.paths.gz"],
    capture_output=True,
).stdout
paths = gzip.decompress(raw).decode().splitlines()
urls = ["https://data.commoncrawl.org/" + p for p in paths
        if "/subset=warc/" in p and p.endswith(".parquet")]
print(f"{crawl}: {len(urls)} warc parquet files", flush=True)

DOMAINS = ["zohorecruit.in", "zohorecruit.com", "zohorecruit.eu", "freshteam.com",
           "darwinbox.in", "darwinbox.com", "keka.com", "greythr.com", "peoplestrong.com",
           "jobsoid.com", "ripplehire.com", "turbohire.co", "qandle.com", "beehivehcm.com",
           "workable.com", "recruitee.com"]
dlist = ",".join(f"'{d}'" for d in DOMAINS)

con = duckdb.connect()
con.execute("INSTALL httpfs; LOAD httpfs;")
con.execute("SET threads=1;")            # fully sequential -> one request at a time, gentlest on CC
...
print(f"total tenant hosts: {sum(n for _, n in rows)} | elapsed: {round(time.time()-t, 1)}s", flush=True)
