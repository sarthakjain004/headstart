...
async def resolve_row(
    ...
    *this* job; a challenged page must not freeze a snippet) that a second copy would drift.
    ...
    try:
        ...
        # A failed fetch is NOT a mismatch, and conflating the two hid the real failure for a
        # whole run: an empty page makes `job_id not in page_html` true, so every timeout was
        # counted as "posting id mismatch". A 2026-08-28 run reported 2,958 of 3,427 fetches
        # mismatched — a number that sent three separate investigations after stale reads, when
        # what was actually happening was `PageLoadTimeout` after pydoll's 300s `go_to` default.
        ...
        return None
    if on_progress:
        on_progress()  # the fetch returned, challenged or not: the tab is alive
