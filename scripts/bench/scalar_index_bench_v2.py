#!/usr/bin/env python3
"""High-fidelity scalar-index bench for the search path's real filter clauses.
...
1. **Real filter strings.** Every clause is produced by :func:`headstart.search_filters.build_filter`
...
2. **ABAB, not AB.** Measuring every no-index case, then building indexes, then measuring every
   indexed case confounds warm-cache drift over the run's lifetime with the index effect. This
   does no-index / indexed / no-index / indexed for each filter and reports both deltas, so a
   monotonic drift (not caused by the index) shows up as two *different* deltas rather than one
...
SCALAR_INDEXES: list[tuple[str, str]] = [
    ...
    # `country` (ADR-0138) is deliberately absent: that change ships with no index, so its case
