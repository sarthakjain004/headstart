...
any agent carrying a domain or an email. Reasoning would not have found that; only running every
scraper did. Without this sweep the fix for one silent multi-run data loss would have shipped
another.

**How it measures.** One live hiring Board per ATS from `data/validate/liveness/`, scraped twice
through the **real** `fetch_raw()` + `parse()` — once under each string — and the Job counts
compared. Boards are drawn small (1-20 jobs) so a full sweep is minutes, and the seed is fixed so a
verdict is reproducible; `--seed` draws a different sample when a second opinion is wanted.
...
def main() -> int:
    ...
    print(
        ...
        f"{len(picks)} Board(s) across {len({a for a, _ in picks})} ATS(es)\n"
        f"  old: {args.old!r}\n  new: {args.new!r}\n",
        flush=True,
    )
    ...
    for ats, slug in picks:
        counts = {}
        for label, agent in (("old", args.old), ("new", args.new)):
        ...
        if not isinstance(old, int) and not isinstance(new, int):
            # Never reached a host under either string — it says nothing about the change, so it
            # must not be counted as evidence that the change is safe.
            verdict = "UNREACHED"
        elif old == new:
            verdict = "SAME"
        elif isinstance(new, int) and (not isinstance(old, int) or new > old):
            verdict = "BETTER"
        else:
            verdict = "WORSE"
