...
Run: python -u scripts/validate/gate_impact.py
     python -u scripts/validate/gate_impact.py --baseline ratio-only   # what ADR-0145 changed
"""
...
def _ratio_only(
    ...
    """The pre-ADR-0145 rule: score over seconds, with no veto on a measured zero.
...
def main() -> int:
    ...
    # and only the first can be stale in the way ADR-0145's ordering note describes.
    ...
    if stale_risk:
        print(
            ...
            "measured into the ledger before merging a gate change (ADR-0145, 'the ordering is "
            "load-bearing').",
            flush=True,
        )
    return 0
