"""Minimal localhost UI for the Sidecorpus semantic search (dev demo).

Loads nomic + the LanceDB ``sidecorpus`` table once at startup, serves a search page at ``/`` and a
...
min_years), rank the survivors by cosine. Sidecorpus-only — that's the only table in the index.
...
<style>
  ...
  * { box-sizing:border-box; }
  ...
  <p class="sub">nomic embeddings → LanceDB filter-then-rank · Sidecorpus corpus (English)</p>
  <div class="bar">
